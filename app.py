...
def find_word():
    if request.method == 'POST':  # Если запрос методом POST
        ...
        if file and allowed_file(file.filename):  # Если файл допустимого формата
            ...
            return render_template('upload_file.html', error='Поддерживаются только файлы формата .docx и .txt') # если загруженный файл не является допустимым, отображается шаблон "upload.html" с сообщением об ошибке.
